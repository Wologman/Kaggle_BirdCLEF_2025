...
class FilePaths:
    def __init__(self, options=None):
        ...
        self.PRETRAIN_DATA = self.DATA_DIR / 'BC_21_23_Audio'
        self.PRETRAIN_AUDIO = self.DATA_DIR / 'BC_21_23_Audio' / 'train_audio'
        self.PRETRAIN_LABELS_PATH = self.PRETRAIN_DATA / 'start_end_8sec_21_23.csv'
        self.LABELS_PATH = self.PRETRAIN_DATA / 'start_end_8sec_21_23.csv'

        self.PRETRAIN_DATA = self.DATA_DIR / 'BC_21_23_Audio'
        self.PRETRAIN_AUDIO = self.DATA_DIR / 'BC_21_23_Audio' / 'train_audio'
        self.PRETRAIN_LABELS_PATH = self.PRETRAIN_DATA / 'start_end_8sec_21_23.csv'
        self.FINETUNE_LABELS_PATH = self.TRAIN_METADATA / 'train.parquet'
...
class TrainingParameters:
    def __init__(self, options=None):
        ...
        self.LR = 1e-3
        self.MIN_LR = 1e-5
        self.LR_CYCLE_LENGTH = 25
        self.LR_DECAY = 0.4
        self.EPOCHS_TO_UNFREEZE_BACKBONE = 4
        self.START_BACKGROUNDS_EPOCH = None
        self.PSEUDOLABEL_INITIAL_WEIGHT = None #0.3
        ...
        self.FOCAL_GAMMA = 2
        self.FIRST_AUGMENTATION_UPDATE = 3
        self.SECOND_AUGMENTATION_UPDATE = 8                  
...
class DefaultAudio:
    ...
    CHUNK_WIDTH = 512 #IMAGE_WIDTH if IMAGE_SHAPE[1] == 1 else IMAGE_WIDTH // 2  #Number of frames wide for each sub-image
    N_MELS = 128 #IMAGE_WIDTH // 2 if IMAGE_SHAPE[0] == 2 else IMAGE_WIDTH #Height of the chunk spectrograms
    N_FFT = 2048 #3072 #2048 *2 #3072 or 2048 #N_fft/2 + 1 bins will get made prior to downsampling to the value of N_MELS
    DOUBLE_AUDIO = True
    BUFFER_AUDIO = 1
    FMIN = 20
    FMAX = 14000 
    HOP_LENGTH = 750 #750 #1500 #1243  #1500  #
...
def backmix(waves: torch.Tensor, backgrounds: torch.Tensor, max_percent: float = 1.0, min_percent: float = 0.3):
    ...
    # Mix waveforms
    waves = coeffs_1 * waves + coeffs_2 * backgrounds
    eps = 1e-8
    max_vals = waves.abs().amax(dim=1, keepdim=True)
    waves = waves / (max_vals + eps)
...
def get_backgrounds(path_list, batch_size, duration):
...
class BirdSoundModel(pl.LightningModule):
    ...
    class AttentionBlock(nn.Module):
        ...
        def forward(self, x):
            ...
            norm_att = torch.softmax(torch.tanh(attn), dim=-1)/(16//split_length)  #so that they have a mean value of 1/16 each
    ...
    def __init__(self,
        ...
        self.pseudolabel_weight = cfg.PSEUDOLABEL_INITIAL_WEIGHT
    ...
    def training_step(self, batch, batch_idx):
        ...
        num_sumix = random.choice([0,1,2])

        if num_sumix == 1:
            audios, targets = sumix(audios, targets)
        elif num_sumix == 2:
            audios, targets = sumix(audios, targets)
            audios, targets = sumix(audios, targets)
        ...
        if self.pseudolabel_weight is not None: 
            if self.current_epoch >= self.start_backgrounds_epoch: #  and random.random() < .2:
                ...
                padding_rows = batch_size - background_size
                if padding_rows > 0:
                    ...
                    background_zeros = torch.zeros((padding_rows, backgrounds.shape[1]), device=background_preds.device, dtype=background_preds.dtype)
                    backgrounds = torch.cat([backgrounds, background_zeros], dim=0)

                zero_threshold = 0.1
                k=0.1
                final_weight = self.pseudolabel_weight + 0.1
                weight = final_weight - (final_weight - self.pseudolabel_weight) * math.exp(-k * max(0,self.current_epoch-self.start_backgrounds_epoch))
                zero_mask = background_preds >= zero_threshold  # shape [batch_size, num_classes], bool
                scaled_labels = torch.where(zero_mask, background_preds, torch.zeros_like(background_preds))*weight   # How about doing the high prob equivalent.  Scaling all the high probability targets to 1 for example
                targets = targets + scaled_labels * (1 - targets)  #The goal here is not to create excessive loss with the uncertain backgrounds, except from difference by adding with labelled data.
                audios = backmix(audios, backgrounds, max_percent=1, min_percent=0.3)
...
def run_training(dl_train, dl_val, data_cfg, train_cfg, audio_cfg, checkpoint_dir):
    ...
    audio_model = BirdSoundModel(train_cfg,
                                 ...
                                 backbone_checkpoint_path='/home/olly/Desktop/Kaggle_BC25/Data/Experiments/Exp_62_pretrain/Results/epoch=14-step=40440.ckpt',
                                 )
    ...
    checkpoint_callback = ResettingModelCheckpoint(
...
def load_training_data(labels_path, data_path, excluded_classes, use_secondary):
    ...
    in_df['filepath'] = str(data_path) + '/' + in_df['primary_label'] + '/' + in_df['filename']

    print(in_df['filepath'].to_list()[:5])
    ...
    if 'centres' in list(in_df.columns):
...
in_df = load_training_data(paths.LABELS_PATH, paths.PRETRAIN_AUDIO, data_cfg.EXCLUDED_CLASSES, data_cfg.USE_SECONDARY) #MAYBE GET RID OF USE_SECONDARY?  HANDLE WITH WEIGHTS INSTEAD
print(in_df.head())

in_df = in_df[~in_df['filename'].isin(['houspa/XC584512_2.ogg'])]

print(f'The shape after filtering is {in_df.shape}')

top_labels = in_df['primary_label'].value_counts().nlargest(206).index

# Filter the DataFrame to only include those labels
in_df = in_df[in_df['primary_label'].isin(top_labels)]

print(f'The shape after keeping only the top 206 labels is {in_df.shape}')
...
short_df = None #short_df[~short_df['original_file'].isin(val_files)]
...
if short_df is not None:
    ...
    train_df_short = train_df[train_df['filepath'].isin(short_df_files)]
else:
    train_df_short = None
...
if (__name__ == '__main__') and (train_cfg.TRAIN):
    metrics = run_training(dl_train, dl_val, data_cfg, train_cfg, audio_cfg, paths.chkpt_dir) 
